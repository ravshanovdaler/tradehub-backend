...
class CRMCalculationsTestCase(APITestCase):
    ...
    def test_crm_stats_dynamic_product_mfg_and_order_transport(self):
        ...
        OrderItem.objects.create(order=order, product=prod_with_mfg, quantity=2, price=10.00)  # mfg: 2 * 4.00 = 8.00
        OrderItem.objects.create(order=order, product=prod_without_mfg, quantity=4, price=20.00) # mfg: 0.00
        ...
        finance = response.data['finance']
        self.assertEqual(finance['manufacturing'], 8.00)
        self.assertEqual(finance['logistics'], 15.00)
        
        # Profit = 100.00 - 8.00 - 15.00 = 77.00
        self.assertEqual(finance['profit'], 77.00)
